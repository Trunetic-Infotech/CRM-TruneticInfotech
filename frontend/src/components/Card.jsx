...
const Card = ({ label, data, icon }) => {
  return (
    <div className="flex flex-col justify-between bg-white shadow-md rounded-2xl p-5 w-full max-w-xs sm:max-w-sm md:max-w-md 
                    transform hover:scale-[1.03] hover:shadow-xl transition duration-300 border-l-4 border-blue-500 animate-fade-in relative">
      {/* Top Section */}
      <div className="flex items-center justify-between">
        <p className="text-sm sm:text-base text-gray-500 font-semibold tracking-wide">{label}</p>
        <div className="p-2 rounded-full bg-blue-100 absolute right-5 top-5">
          <img src={icon} alt="icon" className="w-10 h-10 sm:w-14 sm:h-14" />
        </div>
      </div>
